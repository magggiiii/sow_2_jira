...
# SOW-to-Jira Guided Installer for Ubuntu / Debian
# Optimized for apt-get and systemd
...
# Utility: Confirm with user (Pipe-safe for bash)
confirm() {
...
if ! command -v git &> /dev/null; then
    ...
    if confirm "Would you like me to install Git via apt?"; then
...
if ! command -v docker &> /dev/null; then
    ...
    if confirm "Would you like me to install Docker via the official script?"; then
...
if ! docker info &> /dev/null; then
    ...
    if confirm "Would you like me to start the Docker service (requires sudo)?"; then
        sudo systemctl start docker
        until docker info &> /dev/null; do sleep 2; done
    else
        echo -e "${RED}[ERROR] Docker service must be running.${NC}"
        exit 1
    fi
fi
...
LAUNCH_CMD="alias s2j='export SOW_DATA_HOME=\"$SOW_DATA\" && export SOW_ENV_FILE=\"$GLOBAL_ENV\" && cd \"$SOW_SOURCE\" && docker-compose -f docker-compose.yml up -d && xdg-open http://localhost:8000'"

# Portable update of .bashrc
touch "$SHELL_RC"
